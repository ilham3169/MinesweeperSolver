...
def count(driver):
	...
	y = 0
	counter = 0
	if driver.find_element("id",f"cell_{x}_{y}").get_attribute('class') != "cell size24 hd_flag":
		if driver.find_element("id",f"cell_{x}_{y}").get_attribute('class') == "cell size24 hd_closed":
			counter+=1

	if x-1 >= 0 and y-1>=0:
		if driver.find_element("id",f"cell_{x-1}_{y-1}").get_attribute('class') != "cell size24 flag":
			if driver.find_element("id",f"cell_{x-1}_{y-1}").get_attribute('class') == "cell size24 hd_closed":
				counter+=1

	if y-1 >= 0:
		if driver.find_element("id",f"cell_{x}_{y-1}").get_attribute('class') != "cell size24 hd_flag":
			if driver.find_element("id",f"cell_{x}_{y-1}").get_attribute('class') == "cell size24 hd_closed":
				counter+=1

	if x+1<=8 and y-1 >= 0:
		if driver.find_element("id",f"cell_{x+1}_{y-1}").get_attribute('class') != "cell size24 hd_flag":
			if driver.find_element("id",f"cell_{x+1}_{y-1}").get_attribute('class') == "cell size24 hd_closed":
				counter+=1

	if x+1<=8:
		if driver.find_element("id",f"cell_{x+1}_{y}").get_attribute('class') != "cell size24 hd_flag":
...
def marking(driver):
	...
	for j in range(9):
		for i in range(9):
			if driver.find_element("id",f"cell_{x}_{y}").get_attribute('class') == "cell size24 hd_opened hd_type1":
				"""
def check1(mat):
	for j in range(9):
		for i in range(9):
...
driver.get("https://minesweeper.online/game/2797125350")
time.sleep(5)
